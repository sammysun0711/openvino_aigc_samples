...
def detect(net, img, device):
    ...
    #core = ov.Core()
    #example_inputs = {"x": imgs}
    #if not OV_FACE_DETECTION_MODEL_PATH.exists():
    #    ov_model = ov.convert_model(net, example_input=example_inputs)
    #    ov.save_model(ov_model, OV_FACE_DETECTION_MODEL_PATH)
    #else:
    #    ov_model = core.read_model(OV_FACE_DETECTION_MODEL_PATH)
    #face_detection_compiled_model = core.compile_model(ov_model, "CPU")
...
def batch_detect(net, imgs, device):
    ...
    #with torch.no_grad():
    #    olist_pytorch = net(imgs)
    print("imgs.shape: ", imgs.shape)
    results = net({"x": imgs.numpy()})
    olist = [torch.Tensor(results[i]) for i in range(12)]
    #print("olist_pytorch[0].shape: ", olist_pytorch[0].shape)
    #print("olsit_ov[0].shape: ", olist_ov[0].shape)
    #olist = olist_ov
    bboxlist = []
    for i in range(len(olist) // 2):
        olist[i * 2] = F.softmax(olist[i * 2], dim=1)
...
def face_detect_ov(images, device):
...
def datagen(frames, mels):
    ...
    if args.box[0] == -1:
        if not args.static:
            ...
            face_det_results = face_detect_ov(frames, "CPU")
        else:
            #face_det_results = face_detect([frames[0]])
            face_det_results = face_detect_ov([frames[0]], "CPU")
    else:
...
def main():
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
        ...
        if i == 0:
            ...
            print("mel_batch.shape: ", mel_batch.shape)
            if not OV_WAV2LIP_MODEL_PATH.exists():
                ...
                ov.save_model(ov_model, OV_WAV2LIP_MODEL_PATH)
            else:
                ov_model = core.read_model(OV_WAV2LIP_MODEL_PATH)
            compiled_wav2lip_model = core.compile_model(ov_model, "CPU")
            print("Model loaded")
            ...
            #import openvino as ov
            #example_inputs = {"audio_sequences": mel_batch,
            #                  "face_sequences": img_batch}
            #print("example_inputs: ", example_inputs)
            #ov_model = ov.convert_model(model, example_input=example_inputs)
            #ov.save_model(ov_model, "checkpoints/wav2lip.xml")
        ...
        #else:
        #with torch.no_grad():
        #    pred_torch = model(mel_batch, img_batch)

        pred_ov = compiled_wav2lip_model({"audio_sequences": mel_batch, "face_sequences": img_batch})[0]
        #print("pred_torch shape: ", pred_torch.shape)
        #print("pred_ov shape: ", pred_ov.shape)
        #pred = pred.cpu().numpy().transpose(0, 2, 3, 1) * 255.
        pred_ov = pred_ov.transpose(0, 2, 3, 1) * 255.
        #pred_torch =  pred_torch.cpu().numpy().transpose(0, 2, 3, 1) * 255.
        #print("pred_torch shape: ", pred_torch.shape)
        print("pred_ov shape: ", pred_ov.shape)
