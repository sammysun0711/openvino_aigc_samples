...
def convert_animatediff(
    ...
    if not TEXT_ENCODER_PATH.exists():
        ...
        convert_text_encoder(text_encoder, use_t5_int4=False)
        del text_encoder
        gc.collect()
...
def init_animatediff_pipeline(
    models_dict: Dict[str, Any], device: str):
    pipeline_args = {}
...
if __name__ == "__main__":
    #model_id="Realistic_Vision_V5.1_noVAE"
    #adapter_path="animatediff-motion-adapter-v1-5-2"
    #convert_animatediff(model_id, adapter_path)
    device = "CPU"
    models_dict = {
        "unet": UNET_PATH,
    ...
    ov_pipe = init_animatediff_pipeline(models_dict, device=device)
    ...
    frames = output.frames[0]
    export_to_gif(frames, "animatelcm-motion-lora_ov.gif")
    """
    scheduler = DDIMScheduler.from_pretrained(
        MODEL_DIR,
        subfolder="scheduler",
